import {weatherKey, unsplashKey} from '../secret.js';
...
const weatherApp = async () => {
  ...
  sunrise.innerHTML = `<img src="./asset/001-sunrise.png">
        <p>${getSunriseAndSunsetTime(srTime)}</p>
        <span>Sunrise</span>`;

  sunset.innerHTML = `<img src="./asset/002-sunset.png">
        <p>${getSunriseAndSunsetTime(ssTime)}</p>
        <span>Sunset</span>`;

  windSpeed.innerHTML = `<img src="./asset/001-wind.png">
        <p>${weatherInfo.wind.speed}</p>
        <span>Wind speed</span>`;

  cloudiness.innerHTML = `<img src="./asset/001-clouds.png">
  ...
  document.getElementById(
    'container'
  ).style.cssText = `background: no-repeat url(${unsplash.results[0].urls.raw}); background-size: cover;`;
...
const fetchData = async (inputArea) => {
  try {
    const response = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${inputArea}&appid=${weatherKey}`
    );
...
const changeBackgroundFunc = async (inputArea) => {
  try {
    const response = await fetch(
      `https://api.unsplash.com/search/photos?query=${inputArea}&orientation=portrait&client_id=${unsplashKey}`
    );
